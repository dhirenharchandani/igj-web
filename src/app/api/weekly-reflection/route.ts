...
export async function POST(req: NextRequest) {
  ...
  const [{ data: reset }, { data: weeklyScore }, { data: dailyScores }, { data: profile }] = await Promise.all([
    supabase.from('weekly_resets').select('*').eq('user_id', user.id).eq('week_start', weekStart).single(),
    supabase.from('weekly_scorecards').select('*').eq('user_id', user.id).eq('week_start', weekStart).single(),
